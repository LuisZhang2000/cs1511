#include <stdio.h>
#include <stdlib.h>
#include <assert.h>

struct node {
    struct node *next;
    int          data;
};

int get_middle(struct node *head);
struct node *strings_to_list(int len, char *strings[]);

// DO NOT CHANGE THIS MAIN FUNCTION

int main(int argc, char *argv[]) {
    // create linked list from command line arguments
    struct node *head = strings_to_list(argc - 1, &argv[1]);

    int result = get_middle(head);
    printf("%d\n", result);

    return 0;
}


// Return middle element of a linked list
// if list contains [6,7,8,9,10]  8 is returned
// if a list has even number of elements, first of middle two elements returned
// if list contains [1,2,3,4] 2 is returned
// list can not be empty
int get_middle(struct node *head) {

    int num_elements = 0;
    int count = 0; int midindex = 0;
    int middle = 0;
    struct node *n = head;
    struct node *m = head;

    // Step 1 find total number of elements
    while (n != NULL) {
            num_elements++;
            n = n->next;
        }

    printf("Total Number of elements: %d\n", num_elements);

    // step 2, determine middle index for odd or even
    if (num_elements % 2 == 0) {    // case 1: even num of elements
        printf("even\n");
        midindex = num_elements/2 - 1;
    } else {    // case 2: odd num of elements
        printf("odd\n");
        midindex = (num_elements - 1)/2 ;
    }

    printf("Mid Index is %d\n", midindex);

    // step 3, find the middle number
    while (m != NULL) {
            if (count == midindex) {
                middle = m->data;
            }
            count++;
            m = m->next;
            printf("Middle number is %d\n", middle);

    }


    return middle;
}


// DO NOT CHANGE THIS FUNCTION

// create linked list from array of strings
struct node *strings_to_list(int len, char *strings[]) {
    struct node *head = NULL;
    for (int i = len - 1; i >= 0; i = i - 1) {
        struct node *n = malloc(sizeof (struct node));
        assert(n != NULL);
        n->next = head;
        n->data = atoi(strings[i]);
        head = n;
    }
    return head;
}
